<template>
    <div class="flex items-top gap-2" :class="{ 'mb-2':index != 10 }">
        <div class="flex items-center gap-2">
            <Skeleton v-if="loading && index == 0" class="w-8 h-8 rounded"/>
            <div v-else class="w-8 h-8 border-2 border-slate-800 text-slate-800 rounded flex items-center justify-center">
                {{ index  }}
            </div>
            <Skeleton v-if="loading && story.image == ''" class="min-w-[60px] aspect-[1/1.4]"/>
            <figure v-else>
                <img class="max-w-[60px] min-w-[60px] aspect-[1/1.4] object-cover" :src="story.image" :srcset="story.image" :alt="story.name"/>
            </figure>
        </div>
        <div class="h-full w-full">
            <Skeleton v-if="loading && story.name == ''" class="w-full h-4 mb-1"/>
            ...
            <Skeleton v-if="loading && story.genres.length == 0" class="w-full h-3"/>
            <div v-else class="text-sm font-bold">
                <span>Gêneros: </span>
                <template v-for="genre,index in story.genres" :key="index">
                    <NuxtLink class="text-xs hover:text-cyan-500 inline-block" :to="`/genres/${genre.genre_slug}`">
